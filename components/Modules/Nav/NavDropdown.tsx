...
>(({ children, label, classes, ...props }, forwardedRef) => {
  ...
  return (
    ...
    >
      ...
      >
        {isOpen && (
          <div className={`dropdown-menu ${classes} ${isOpen ? "open" : ""}`}>
            <ul className="dropdown-container" ref={refs.setFloating} {...getFloatingProps()}>
              {children}
            </ul>
          </div>
        )}
...
>(
  (
    { label, href, classes, disabled, image, description, ...props },
    forwardedRef
  ) => {
